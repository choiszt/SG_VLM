...
if __name__ == "__main__":
    ...
    for k in octopus_data['data'].keys():
        ...
        try:
            ...
            tapa_octopus[task_status]['reward']=one_data['reward']
        ...
        for cnt in tqdm(range(len(task["plan"]))):
            ...
            if cnt==0:
                ...
                for i in range(cnt,len(task['plan'])):
                    result_plan+=f"Step {i+1}: {task['plan'][i]}\n"
                result[task_with_step]['answer']=result_plan
            result[task_with_step]['reward']=tapa_octopus[key]['reward']

    with open("./data/Octopus/Octopus_executable_and_unseen_planning.json","w+")as f:
