...
def main(
    test_EXP="initsg_planning",#initsg_finalsg_planning,initsg_finalsg_targetobj_planning,initsg_planning,initsg_targetobj_planning
    *,
    num_samples: int = 1,
    ...
    for task in tqdm(list(OGSG_data.keys())):
        ...
        ori_prompt=OGSG_data[task]['instruction']
        instruct=ori_prompt.split("\n")[-1]
        prompt=ori_prompt.split("Observed Relation:")[0]+instruct
        ...
        # print(output)
        with open(f"Inference_result/LLaMA7B/Goal/{task}.txt","w")as f:
            f.write(output)
