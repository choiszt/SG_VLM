...
EXPERIMENT_NAME="octopus_iterative_plan_0316_lora"
def main(
    data_dir: str = "data/Octopus/Octopus_iterative_executable_planning", 
    pretrained_path: str = "checkpoints/lit-llama/7B/lit-llama.pth",
    out_dir: str =f"out/lora/Octopus/{EXPERIMENT_NAME}",
):


    logger=TensorBoardLogger("./log", name=EXPERIMENT_NAME)
    fabric = L.Fabric(accelerator="cuda", devices=devices,strategy=(DeepSpeedStrategy(config=ds_config) if devices > 1 else "auto"),  precision="bf16-true",loggers=logger)
    fabric.launch()
    ...
    # save_model_checkpoint(fabric, model, os.path.join(out_dir, "lit-llama-lora-finetuned.pth"))
